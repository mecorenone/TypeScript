...
export function createWatchUtils<PollingWatcherData, FsWatcherData>(
    ...
    function initializeWatches<Data>(name: string): Watches<Data> {
        ...
        function forEach(path: string, cb: (data: Data) => void) {
            ...
            canonicalPathsToStrings?.get(getCanonicalFileName(path))?.forEach(canonicalSamePath => {
                if (canonicalSamePath !== path) allData = addRange(allData, actuals.get(canonicalSamePath));
            });
            allData?.forEach(cb);
        }
